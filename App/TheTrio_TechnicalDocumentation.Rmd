...
The layout of our application will consist of two component: Dataset and Model
## 1. Dataset Tab

The sidebar is where the user choose their dataset as well specifying the range of forecast period they are interested in. 
To ensure meaningful forecasting results, we want the forecasting period to include a sufficient number of out-of-sample observations. In this case, we set a minimum of 30 out-of-sample points. Thus, we define a function that can determine the minimum ending period given the chosen starting period. 
```{r}
